...
def login():
    ...
    if result:
        ...
        if bcrypt.check_password_hash(hashed_pw, request.form['loginPassword']):
            session['login'] = True
...
def welcome():
    if not session['login']:
        ...
        mysql2 = connectToMySQL('quotes_dash2')
        quoteText = "SELECT * FROM users LEFT JOIN quotes ON users.id = quotes.user_id ORDER BY quotes.id DESC;"
        ...
        # below is some code that I might uncomment out depending on whether it works in the end
        # mysql3 = connectToMySQL('quote_dash')
        # allTweetsQuery = "SELECT * FROM tweets LEFT JOIN users ON users.id = tweets.user_id ORDER BY tweets.id DESC;"
        # allTweets = mysql3.query_db(allTweetsQuery)

        # mysql4 = connectToMySQL('quotes_dash2')
        # query4 = "SELECT COUNT(likes) FROM likes WHERE quote_id = %(q_id)s;"
        # data4 = {
